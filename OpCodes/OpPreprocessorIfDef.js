...
Object.assign(Bayrell.Lang.OpCodes.OpPreprocessorIfDef,
{
	...
	getFieldInfoByName: function(ctx,field_name)
	{
		var Collection = use("Runtime.Collection");
		var Dict = use("Runtime.Dict");
		if (field_name == "KIND_PROGRAM") return Dict.from({
			"t": "string",
			"annotations": Collection.from([
			]),
		});
		if (field_name == "KIND_CLASS_BODY") return Dict.from({
			"t": "string",
			"annotations": Collection.from([
			]),
		});
		if (field_name == "KIND_OPERATOR") return Dict.from({
			"t": "string",
			"annotations": Collection.from([
			]),
		});
		if (field_name == "KIND_EXPRESSION") return Dict.from({
			"t": "string",
			"annotations": Collection.from([
			]),
		});
