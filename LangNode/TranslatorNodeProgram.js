...
Object.assign(Bayrell.Lang.LangNode.TranslatorNodeProgram,
{
	...
	translateProgramHeader: function(__ctx, t, op_code)
	{
		...
		content += use("Runtime.rtl").toStr(t.s(__ctx, "var use = require('bayrell').use;"));
	...
	OpDeclareClassFooter: function(__ctx, t, op_code)
	{
		...
		content += use("Runtime.rtl").toStr(t.s(__ctx, "if (module.exports == undefined) module.exports = {};"));
		...
		for (var i = 0;i < arr.count(__ctx);i++)
		{
			...
			content += use("Runtime.rtl").toStr((content == 0) ? s : t.s(__ctx, s));
		}
		content += use("Runtime.rtl").toStr(t.s(__ctx, "module.exports." + use("Runtime.rtl").toStr(t.current_class_full_name) + use("Runtime.rtl").toStr(" = ") + use("Runtime.rtl").toStr(t.current_class_full_name) + use("Runtime.rtl").toStr(";")));
		return use("Runtime.Collection").from([t,content]);
	},
