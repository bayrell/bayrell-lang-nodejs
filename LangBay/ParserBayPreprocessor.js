...
Object.assign(Bayrell.Lang.LangBay.ParserBayPreprocessor,
{
	...
	readPreprocessorSwitch: function(__ctx, parser)
	{
		...
		parser = parser.copy(__ctx, { "vars": parser.vars.concat(__ctx, use("Runtime.Dict").from({"ES6":true,"NODEJS":true,"JAVASCRIPT":true,"PHP":true,"PYTHON3":true})) });
		...
		while (token.content == "#case")
		{
			...
			var res = parser.parser_base.constructor.readUntilStringArr(__ctx, parser, use("Runtime.Collection").from(["#case","#endswitch"]), false);
			parser = res[0];
			content = res[1];
			/* Look content */
			var res = parser.parser_base.constructor.readToken(__ctx, parser.clone(__ctx));
			...
			var ifcode = new __v0(__ctx, use("Runtime.Dict").from({"condition":condition,"content":content,"caret_start":caret_content,"caret_end":parser.caret.clone(__ctx)}));
		...
		return use("Runtime.Collection").from([parser,new __v0(__ctx, use("Runtime.Dict").from({"items":items.toCollection(__ctx),"caret_start":caret_start,"caret_end":parser.caret.clone(__ctx)}))]);
	},
