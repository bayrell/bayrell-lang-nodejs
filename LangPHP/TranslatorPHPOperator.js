...
Object.assign(Bayrell.Lang.LangPHP.TranslatorPHPOperator,
{
	...
	OpReturn: function(__ctx, t, op_code)
	{
		...
		if (t.current_function.flags != null && t.current_function.flags.isFlag(__ctx, "memorize"))
		{
			...
			content += use("Runtime.rtl").toStr(t.s(__ctx, t.expression.constructor.getModuleName(__ctx, t, "Runtime.rtl") + use("Runtime.rtl").toStr("::_memorizeSave(\"") + use("Runtime.rtl").toStr(t.current_class_full_name) + use("Runtime.rtl").toStr(".") + use("Runtime.rtl").toStr(t.current_function.name) + use("Runtime.rtl").toStr("\", func_get_args(), $__memorize_value);")));
	...
	OpThrow: function(__ctx, t, op_code)
	{
		var res = t.expression.constructor.Expression(__ctx, t, op_code.expression);
		t = res[0];
		var content = t.s(__ctx, "throw " + use("Runtime.rtl").toStr(res[1]) + use("Runtime.rtl").toStr(";"));
		return use("Runtime.Collection").from([t,content]);
	},
	/**
	 * OpTryCatch
	...
	OpTryCatch: function(__ctx, t, op_code)
	{
		...
		t = t.levelDec(__ctx);
		content += use("Runtime.rtl").toStr(t.s(__ctx, "}"));
		content += use("Runtime.rtl").toStr(t.s(__ctx, "catch (\\Exception $_ex)"));
		content += use("Runtime.rtl").toStr(t.s(__ctx, "{"));
		t = t.levelInc(__ctx);
		for (var i = 0;i < op_code.items.count(__ctx);i++)
		{
			...
			s += use("Runtime.rtl").toStr((s != "") ? t.s(__ctx, "$" + use("Runtime.rtl").toStr(item.name) + use("Runtime.rtl").toStr(" = $_ex;")) : "$" + use("Runtime.rtl").toStr(item.name) + use("Runtime.rtl").toStr(" = $_ex;"));
	...
	Operator: function(__ctx, t, op_code)
	{
		...
		else if (op_code instanceof __v1)
		{
			...
			content += use("Runtime.rtl").toStr(t.s(__ctx, "$" + use("Runtime.rtl").toStr(op_code.var_name) + use("Runtime.rtl").toStr(" = ") + use("Runtime.rtl").toStr(s1) + use("Runtime.rtl").toStr(";")));
			t = t.copy(__ctx, { "save_op_codes": save_op_codes });
			t = t.copy(__ctx, { "save_op_code_inc": save_op_code_inc });
			return use("Runtime.Collection").from([t,content]);
		}
	...
	OpDeclareFunctionBody: function(__ctx, t, f)
	{
		...
		else if (f.expression)
		{
			...
			if (f.flags != null && f.flags.isFlag(__ctx, "memorize"))
			{
				...
				s += use("Runtime.rtl").toStr(t.s(__ctx, t.expression.constructor.getModuleName(__ctx, t, "Runtime.rtl") + use("Runtime.rtl").toStr("::_memorizeSave(\"") + use("Runtime.rtl").toStr(t.current_class_full_name) + use("Runtime.rtl").toStr(".") + use("Runtime.rtl").toStr(f.name) + use("Runtime.rtl").toStr("\", func_get_args(), $__memorize_value);")));
		...
		{
			var s = "";
			s += use("Runtime.rtl").toStr(t.s(__ctx, "$__memorize_value = " + use("Runtime.rtl").toStr(t.expression.constructor.getModuleName(__ctx, t, "Runtime.rtl")) + use("Runtime.rtl").toStr("::_memorizeValue(\"") + use("Runtime.rtl").toStr(t.current_class_full_name) + use("Runtime.rtl").toStr(".") + use("Runtime.rtl").toStr(f.name) + use("Runtime.rtl").toStr("\", func_get_args());")));
			s += use("Runtime.rtl").toStr(t.s(__ctx, "if ($__memorize_value != " + use("Runtime.rtl").toStr(t.expression.constructor.getModuleName(__ctx, t, "Runtime.rtl")) + use("Runtime.rtl").toStr("::$_memorize_not_found) return $__memorize_value;")));
			content = s + use("Runtime.rtl").toStr(content);
		}
		t = t.levelDec(__ctx);
