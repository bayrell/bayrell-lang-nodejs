...
Object.assign(Bayrell.Lang.LangES6.TranslatorES6Expression,
{
	...
	OpIdentifier: function(__ctx, t, op_code)
	{
		...
		else if (t.modules.has(__ctx, op_code.value) || op_code.kind == __v2.KIND_SYS_TYPE)
		{
	...
	Dynamic: function(__ctx, t, op_code)
	{
		...
		else if (op_code instanceof __v1)
		{
			...
			else if (op_code_first instanceof __v4)
			{
				...
				else if (op_code_first.kind == __v7.KIND_CONTEXT && op_code_first.value == "@")
				{
					prev_kind = "var";
					s = "__ctx";
				}
				else if (op_code_first.kind == __v8.KIND_CONTEXT && op_code_first.value == "_")
				{
					prev_kind = "var";
					s = "__ctx.translate";
				}
	...
	OpMath: function(__ctx, t, op_code)
	{
		...
		else
		{
			...
			if (op_math == "~")
			{
				content = op1 + use("Runtime.rtl").toStr(" ") + use("Runtime.rtl").toStr(op) + use("Runtime.rtl").toStr(" ") + use("Runtime.rtl").toStr(this.rtlToStr(__ctx, t, op2));
			}
	...
	OpTypeConvert: function(__ctx, t, op_code)
	{
		...
		t = res[0];
		var value = res[1];
		content = this.useModuleName(__ctx, t, "rtl") + use("Runtime.rtl").toStr(".to(") + use("Runtime.rtl").toStr(value) + use("Runtime.rtl").toStr(", ") + use("Runtime.rtl").toStr(this.toPattern(__ctx, t, op_code.pattern)) + use("Runtime.rtl").toStr(")");
		return use("Runtime.Collection").from([t,content]);
	},
	/**
	 * OpTernary
